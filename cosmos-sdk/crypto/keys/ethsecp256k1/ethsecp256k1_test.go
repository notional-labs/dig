...
import (
	...
	btcethsecp256k1 "github.com/btcsuite/btcd/btcec"
	"github.com/btcsuite/btcutil/base58"
	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
	"github.com/tendermint/tendermint/crypto"
	tmethsecp256k1 "github.com/tendermint/tendermint/crypto/secp256k1"
...
func TestPubKeyethsecp256k1Address(t *testing.T) {
	for _, d := range secpDataTable {
		...
		var priv = ethsecp256k1.PrivKey{Key: privB}

		pubKey := priv.PubKey()
		pubT, _ := pubKey.(*ethsecp256k1.PubKey)

		addr := pubKey.Address()
		assert.Equal(t, pubT, &ethsecp256k1.PubKey{Key: pubB}, "Expected pub keys to match")
		assert.Equal(t, addr, addrB, "Expected addresses to match")
	}
}

func TestSignAndValidateethsecp256k1(t *testing.T) {
	privKey := ethsecp256k1.GenPrivKey()
	...
	btcPrivKey, btcPubKey := btcethsecp256k1.PrivKeyFromBytes(btcethsecp256k1.S256(), privKey.Key)
	// This fails: malformed signature: no header magic
	//   btcSig, err := ethsecp256k1.ParseSignature(sig, ethsecp256k1.S256())
...
func Testethsecp256k1LoadPrivkeyAndSerializeIsIdentity(t *testing.T) {
	numberOfTests := 256
	for i := 0; i < numberOfTests; i++ {
		...
		priv, _ := btcethsecp256k1.PrivKeyFromBytes(btcethsecp256k1.S256(), privKeyBytes[:])
...
func TestGenPrivKeyFromSecret(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			gotPrivKey := ethsecp256k1.GenPrivKeyFromSecret(tt.secret)
...
func TestPubKeyEquals(t *testing.T) {
	ethsecp256k1PubKey := ethsecp256k1.GenPrivKey().PubKey().(*ethsecp256k1.PubKey)
	...
	testCases := []struct {
	...
	}{
		{
			"different bytes",
			ethsecp256k1PubKey,
			ethsecp256k1.GenPrivKey().PubKey(),
			false,
		},
		{
			"equals",
...
func TestPrivKeyEquals(t *testing.T) {
	ethsecp256k1PrivKey := ethsecp256k1.GenPrivKey()
...
func TestMarshalAmino_BackwardsCompatibility(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			"ethsecp256k1 private key, binary",
			tmPrivKey,
			privKey,
			aminoCdc.Marshal,
		},
		{
			"ethsecp256k1 private key, JSON",
			tmPrivKey,
			privKey,
			aminoCdc.MarshalJSON,
		},
		{
			"ethsecp256k1 public key, binary",
			tmPubKey,
			pubKey,
			aminoCdc.Marshal,
		},
		{
			"ethsecp256k1 public key, JSON",
			tmPubKey,
			pubKey,
			aminoCdc.MarshalJSON,
		},
