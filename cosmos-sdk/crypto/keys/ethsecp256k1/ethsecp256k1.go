...
func (privKey *PrivKey) PubKey() cryptotypes.PubKey {
	_, pubkeyObject := ethsecp256k1.PrivKeyFromBytes(ethsecp256k1.S256(), privKey.Key)
...
// GenPrivKey generates a new ECDSA private key on curve ethsecp256k1 private key.
// It uses OS randomness to generate the private key.
func GenPrivKey() *PrivKey {
	return &PrivKey{Key: genPrivKey(crypto.CReader())}
}

// genPrivKey generates a new ethsecp256k1 private key using the provided reader.
func genPrivKey(rand io.Reader) []byte {
	...
	for {
		...
		isValidFieldElement := 0 < d.Sign() && d.Cmp(ethsecp256k1.S256().N) < 0
...
func GenPrivKeyFromSecret(secret []byte) *PrivKey {
	...
	n := new(big.Int).Sub(ethsecp256k1.S256().N, one)
	fe.Mod(fe, n)
	fe.Add(fe, one)
...
func (pubKey *PubKey) Address() crypto.Address {
	...
	return crypto.Address(append(ethcrypto.PubkeyToAddress(*pubk).Bytes(), 0))
}
